...
$execute if score @s jkbw.Player.Own$(Res)s >= #armor$(id) jkbw.mem run tag @s[tag=!jkbw_bought] add jkbw_buy_success
$execute if score @s jkbw.Player.OwnExpLevelsReal >= #Earmor$(id) jkbw.mem run tag @s[tag=!jkbw_bought] add jkbw_buy_success

# 失败购买（不够物资）
$execute if score #res_mode jkbw.mem matches 0 as @s[tag=!jkbw_bought,tag=!jkbw_buy_success] run scoreboard players operation #shop_temp jkbw.mem = @s jkbw.Player.Own$(Res)s
...
$execute if score #res_mode jkbw.mem matches 0 run scoreboard players operation @s[tag=!jkbw_bought,tag=jkbw_buy_success] jkbw.Player.Own$(Res)s -= #armor$(id) jkbw.mem
