# 检测能否买
tag @s[scores={jkbw.Player.Compass=6}] add jkbw_bought
...
# 失败购买
tellraw @s[tag=jkbw_bought] [{"storage":"jk:bw","nbt":"txt.print.buy_ed","color":"red"},{"storage":"jk:bw","nbt":"txt.display.team.white","color":"yellow"}," ",{"storage":"jk:bw","nbt":"txt.compass.track","color":"yellow"},"!"]
playsound entity.villager.no player @s[tag=jkbw_bought]

# 成功购买
scoreboard players set @s[tag=!jkbw_bought,tag=jkbw_buy_success] jkbw.Player.Compass 6
tellraw @s[tag=!jkbw_bought,tag=jkbw_buy_success] [{"storage":"jk:bw","nbt":"txt.print.buy_ok","color":"green"}," ",{"storage":"jk:bw","nbt":"txt.display.team.white","color":"gold"},"!"]
