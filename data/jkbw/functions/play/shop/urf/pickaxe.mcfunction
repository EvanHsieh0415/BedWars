...
# 成功购买
$execute unless score #attack_mode jkbw.mem matches 1 run give @s[tag=jkbw_buy_success] $(item)_pickaxe{CanDestroy: ["#jkbw:candestroy"], Enchantments: [{id: "efficiency", lvl: 1s}], HideFlags: 28}
$execute if score #attack_mode jkbw.mem matches 1 run give @s[tag=jkbw_buy_success] $(item)_pickaxe{CanDestroy: ["#jkbw:candestroy"], Enchantments: [{id: "efficiency", lvl: 1s}], HideFlags: 28}
