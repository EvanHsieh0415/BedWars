# 检测能否买
execute if entity @e[type=text_display,tag=jkbw_spawn_red,limit=1,scores={jkbw.Team.Healing=1}] run tag @s add jkbw_bought
...
# 成功购买
execute as @s[tag=!jkbw_bought,tag=jkbw_buy_success] run scoreboard players set @e[type=text_display,tag=jkbw_spawn_red,limit=1] jkbw.Team.Healing 1
execute as @s[tag=!jkbw_bought,tag=jkbw_buy_success] run tellraw @a[team=jkbw.red] [{"selector":"@s"}," ",{"storage":"jk:bw","nbt":"txt.print.buy_ed2","color":"green"}," ",{"storage":"jk:bw","nbt":"txt.team.shop.name.healing","color":"gold"}]
