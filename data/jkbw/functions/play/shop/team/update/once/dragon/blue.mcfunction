# 检测能否买
execute if entity @e[type=text_display,tag=jkbw_spawn_blue,limit=1,scores={jkbw.Team.Dragon=1}] run tag @s add jkbw_bought
execute if score #time_state jkbw.mem matches 6.. if entity @e[type=text_display,tag=jkbw_spawn_blue,limit=1,scores={jkbw.Team.Dragon=0}] run tag @s add jkbw_bought
...
# 失败购买
execute if entity @e[type=text_display,tag=jkbw_spawn_blue,limit=1,scores={jkbw.Team.Dragon=1}] run tellraw @s[tag=jkbw_bought] [{"storage":"jk:bw","nbt":"txt.print.buy_cannot","color":"red"}," ",{"storage":"jk:bw","nbt":"txt.team.shop.name.dragon","color":"yellow"},", ",{"storage":"jk:bw","nbt":"txt.print.because_lvl_max_team2"}]
execute if score #time_state jkbw.mem matches 6.. if entity @e[type=text_display,tag=jkbw_spawn_blue,limit=1,scores={jkbw.Team.Dragon=0}] run tellraw @s[tag=jkbw_bought] [{"storage":"jk:bw","nbt":"txt.print.buy_cannot","color":"red"}," ",{"storage":"jk:bw","nbt":"txt.team.shop.name.dragon","color":"yellow"},", ",{"storage":"jk:bw","nbt":"txt.print.because_over_time"}]
...
# 成功购买
execute as @s[tag=!jkbw_bought,tag=jkbw_buy_success] run scoreboard players set @e[type=text_display,tag=jkbw_spawn_blue,limit=1] jkbw.Team.Dragon 1
execute as @s[tag=!jkbw_bought,tag=jkbw_buy_success] run tellraw @a[team=jkbw.blue] [{"selector":"@s"}," ",{"storage":"jk:bw","nbt":"txt.print.buy_ed2","color":"green"}," ",{"storage":"jk:bw","nbt":"txt.team.shop.name.dragon","color":"gold"}]
