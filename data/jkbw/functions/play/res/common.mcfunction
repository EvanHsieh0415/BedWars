...
$execute if score #summon_$(res) jkbw.mem matches 1 as @e[type=text_display,tag=jkbw_$(res)_time] at @s unless entity @e[type=item,tag=jkbw_extra_$(res),dx=0,dy=-8,dz=0] run summon item ~ ~ ~ {Age: -32768, PickupDelay: 18, Invulnerable: True, Item: {id: "$(res)", Count: 1, tag: {CanPlaceOn: ["#jkbw:canplaceon"], CanDestroy: ["#jkbw:candestroy"], HideFlags: 28}}, Tags: ["jkbw", "jkbw_res", "jkbw_res_$(res)"]}
